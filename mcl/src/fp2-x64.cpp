/*
	imported from https://github.com/herumi/ate-pairing
*/
#include <iostream>
...
// rr = r^(-1) % p
mpz_class Fp::p_;
mpz_class Fp::montgomeryR_;
mpz_class Fp::p_add1_div4_;
Fp Fp::montgomeryR2_;
Fp Fp::one_;
...
//		in_FpDbl_addNC(2, T3, T3, T2); // RRR?
	...
	*/
	...
	void init(const mpz_class& p, uint64_t pp)
	{
		pp_ = pp;
		set_p_Fp_mul();
		set_p_Fp2_neg();
...
void Fp::setTablesForDiv(const mpz_class& p)
{
	// for divBy2
	assert((p[0] & 0x1) == 1);
	halfTbl_[0].clear();
	...
	// for divBy4
	assert((p[0] & 0x3) == 3);
	mpz_class quarter = (p+1)>>2;
}

void Fp::setModulo(const mpz_class& p, int mode, bool useMulx, bool definedBN_SUPPORT_SNARK)
{
#ifdef BN_SUPPORT_SNARK
	...
	{
		typedef mie::ZmZ<mpz_class, MontgomeryDummy> Z;
		Z::setModulo(p);
		Z t(montgomeryR_);
